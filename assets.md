...
- В трёхколоночной галерее архетипа компонент пропорционально уменьшается до строки и блока стоимости `40px`, блока количества `30px` и отступа арта `27px`. Полноразмерная одиночная колода сохраняет исходную геометрию.
