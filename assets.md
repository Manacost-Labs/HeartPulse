...
- Структура и внешний вид совпадают с vendored `hsreplay-deck-view`: слева плоский блок стоимости в цвете редкости, затем тёмная рамка, прямоугольный арт с `65deg`-переходом и правый блок количества.
- Базовая ширина — `486px`, высота строки — `62px`; на экране до `520px` высота уменьшается до `52px`. Размеры gem/count/art offset должны браться из `--hsrdv-*` переменных исходного компонента.
- `assets/mana.png`, классовая тонировка, кованые рамки, CSS-маски арта и собственные rarity-цвета в этом компоненте не применяются.
- Количество `2+` выводится золотым числом `#f7db48`; единственная легендарная карта получает золотую звезду. Название занимает одну строку с ellipsis и исходной чёрной обводкой.
- В трёхколоночной галерее архетипа компонент пропорционально уменьшается до строки и блока стоимости `40px`, блока количества `30px` и отступа арта `27px`. Сначала показываются пять строк, полный состав раскрывается отдельной кнопкой. Фан-колоды используют отдельную шестиколоночную сетку и сверхкомпактную плотность: все строки основной колоды и сайдборда видны сразу, высота строки и блока стоимости равна `25px`, блок количества — `18px`. Арт занимает 90% внутренней рамки: это прячет светлый левый край, который уже присутствует в некоторых исходных `tiles/*.webp` HearthstoneJSON, под непрозрачной частью fade. Новые изображения не требуются: используются существующие арты карт, rarity-цвета vendored-компонента и `wallpaper/deck-border.png` для внешней рамы.
- Интерактивный конструктор сохраняет минимальную высоту `44px` для кнопок, но не меняет визуальную модель HSReplay.
