...
- В трёхколоночной галерее архетипа компонент пропорционально уменьшается до строки и блока стоимости `40px`, блока количества `30px` и отступа арта `27px`. Сначала показываются пять строк, полный состав раскрывается отдельной кнопкой; тот же паттерн применяется в карточках фан-колод.
