...
- В трёхколоночной галерее архетипа компонент пропорционально уменьшается до строки и блока стоимости `40px`, блока количества `30px` и отступа арта `27px`. Сначала показываются пять строк, полный состав раскрывается отдельной кнопкой. Фан-колоды используют отдельную шестиколоночную сетку и HSGuru-плотность: все строки основной колоды и сайдборда видны сразу, высота строки и блока стоимости равна `29px`, блок количества — `21px`. Новые изображения не требуются: используются существующие арты карт, rarity-цвета vendored-компонента и `wallpaper/deck-border.png` для внешней рамы.
