...
В плотном списке просмотра кристалл вписывается в строку `28px` (`27px` на узком экране) через `background-size: min(32px, calc(var(--deck-tile-height) + 2px))`. Интерактивный конструктор сохраняет строки и элементы управления высотой не менее `44px`.
