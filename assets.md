...
- Референсная геометрия исходного компонента — `486px` ширины и `62px` высоты. Все публичные неинтерактивные экземпляры задают через `--hsrdv-*` единый компактный профиль: строка и блок стоимости `25px`, блок количества `18px`, отступ арта справа `16px`, отступ имени `4px`.
...
- Единый компактный профиль действует в галереях и деталях архетипов, составе колоды в мете, деталях карт и фан-колодах. Арт занимает 90% внутренней рамки и привязан к правому краю: это прячет светлый левый край, который уже присутствует в некоторых исходных `tiles/*.webp` HearthstoneJSON, под непрозрачной частью fade. В архетипах сначала показываются пять строк и полный состав раскрывается отдельной кнопкой; в шестиколоночной сетке фан-колод все строки основной колоды и сайдборда видны сразу. Новые изображения не требуются: используются существующие арты карт, rarity-цвета vendored-компонента и `wallpaper/deck-border.png` для внешней рамы.
