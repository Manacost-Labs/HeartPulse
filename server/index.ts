...
async function fetchStandardMetaPreview(hash: string): Promise<StandardMetaPreview> {
  ...
  for (const cached of standardMetaPreviewCache.values()) {
    if (cached.expiresAt > now && cached.preview.hash === hash && isDeckviewPreview(cached.preview)) {
...
registerPublicApi({ app, getDatabase: db, adminAuth, adminId: admin => admin.id, setPrivateNoStore, recordAudit: recordAdminAudit, cardImageDependencies: cardImageRouterDependencies, accessTokens: applicationAuth, ...createPublicApiCardSources(() => constructedCardDataService), metaStatistics: { loadMeta: loadStandardMeta, loadCatalog: loadConstructedArchetypeCatalog, loadHistory: loadConstructedArchetypeHistory, loadAnalysis: loadConstructedArchetypeAnalysis }, deckStatistics: { loadCatalog: loadConstructedArchetypeCatalog } });
