import { ensureAccountColumns } from './app/accountColumns.js';
import {
...
import { proxyHsReplayStrategyPayload } from './modules/publicApi/public.js';
import {
...
const { setAuthCookie, clearAuthCookie } = createBrowserSessionCookieHandlers({
  ...
  legacyCookieName: AUTH_LEGACY_COOKIE_NAME,
...
function db(): DatabaseSync {
  ...
  ensureAccountColumns(ecosystemDb);
...
function cookieMutationCsrfAllowed(req: import('express').Request): boolean {
  ...
  return csrfRequestAllowed({
    ...
    authCookiePresent: cookieValues(
      ...
      AUTH_COOKIE_NAME,
    ).length > 0 || cookieValues(String(req.headers.cookie ?? ''), AUTH_LEGACY_COOKIE_NAME).length > 0,
...
const browserIdentity = registerBrowserIdentity({ app, getDatabase: db, authCookieName: AUTH_COOKIE_NAME, legacyAuthCookieName: AUTH_LEGACY_COOKIE_NAME });
