import { isIndexableConstructedCard, projectPublicConstructedCardSeoData as projectPublicCard, createConstructedCardReader, createPublicCardRouter, type PublicConstructedCardSeoData } from './modules/constructedCards/public.js';
export { isIndexableConstructedCard, type PublicConstructedCardSeoData } from './modules/constructedCards/public.js';
...
export function projectPublicConstructedCardSeoData(card: JsonRecord, origin?: string): PublicConstructedCardSeoData {
  return projectPublicCard(card, origin, sameOriginPublicResourceUrl);
}
...
export function createConstructedCardSeoRouter(dependencies: ConstructedCardSeoRouterDependencies): Router {
  ...
  const readCard = createConstructedCardReader({ ...dependencies, catalogTimeoutMs });
  router.use(createPublicCardRouter(readCard, card => projectPublicConstructedCardSeoData(card, origin)));
  const handler: RequestHandler = async (request, response) => {
    ...
    try {
      const card = await readCard(format, cardId);
      if (!card) {
        const html = renderNoindexDocument({
          title: 'Карта не найдена | HearthPulse',
          description: 'Запрошенная карта Hearthstone не найдена.',
