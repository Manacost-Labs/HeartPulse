...
>(
  ...
  router.get('/ecosystem/internal/subscription', dependencies.internalGuard, asyncRoute(async (request, response) => {
    dependencies.setPrivateNoStore(response);
    const user = dependencies.resolveUser(request);
    if (!user) return response.status(404).json({ error: 'User not found' });
  ...
  router.post('/ecosystem/internal/subscription', dependencies.internalGuard, asyncRoute(async (request, response) => {
    dependencies.setPrivateNoStore(response);
    const user = dependencies.resolveUser(request);
    if (!user) return response.status(404).json({ error: 'User not found' });
