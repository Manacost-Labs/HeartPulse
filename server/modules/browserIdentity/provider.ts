...
function providerConfiguration(options: BrowserIdentityOptions): Configuration {
  return {
    ...
    findAccount: async (_ctx, subject, token) => {
      const account = await options.resolveAccount(subject, token?.grantId);
...
/** Unmounted foundation; composition/epoch and consent integration are mandatory before activation. */
