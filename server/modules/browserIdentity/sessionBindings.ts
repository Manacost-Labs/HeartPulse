...
export function createSessionBindings(database: DatabaseSync, key: Uint8Array, now = Date.now) {
  ...
  database.exec(`CREATE TABLE IF NOT EXISTS browser_identity_session_bindings (
  ...
  return {
    ...
    cleanup(): void {
      database.prepare('DELETE FROM browser_identity_session_bindings WHERE expires_at <= ?').run(now());
    },
  };
}
