...
try {
  ...
  assert.equal((await post('/api/v1/tracker/events/batch', 400)).status, 200);
  assert.equal((await post('/api/v1/tracker/events/batch', 550)).status, 413);
} finally {
