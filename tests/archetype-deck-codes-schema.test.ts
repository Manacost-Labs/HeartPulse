...
try {
  ...
  database.prepare(`
    INSERT INTO archetype_deck_codes VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(
    'weapon rogue', 'Weapon Rogue', 'AAEBA-all', 'wild', 'all', 'hsguru-decks', '2026-07-22T20:30:00Z',
  );
  assert.equal(
    (database.prepare('SELECT rank_key FROM archetype_deck_codes WHERE name_en_key = ?')
      .get('weapon rogue') as { rank_key: string }).rank_key,
    'all',
  );
  ...
  console.log('archetype deck-code all-rank schema migration tests passed');
} finally {
  database.close();
