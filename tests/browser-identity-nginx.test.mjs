...
assert.match(edgeExact, /access_log off;[\s\S]*?error_log \/dev\/null crit;[\s\S]*?return 404;/,
  'bare /identity must be a quiet 404');
...
function fetch(port, path) {
  return new Promise((resolve, reject) => {
    const pending = request({ host: '127.0.0.1', port, path, headers: { Host: 'hearthpulse.net' } }, response => {
      const chunks = [];
...
http {
  ...
  server {
    ...
    ${fixture}
    location = /identity { access_log off; error_log /dev/null crit; return 404; }
...
`);
  ...
  try {
    ...
    assert.equal((await fetch(port, '/identity')).status, 404);
