...
assert.equal(isKnownPath('/privacy'), true, 'privacy policy route must be recognized');
assert.equal(tabFromPath('/terms'), 'terms', 'terms of use must be a standalone public page');
assert.equal(isKnownPath('/terms'), true, 'terms of use route must be recognized');
assert.equal(isKnownPath('/connect'), true, 'application authorization route must be recognized');
