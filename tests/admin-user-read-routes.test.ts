...
app.use('/api', createAdminUserReadRouter({
  ...
  subscriptionForUser: () => ({ hasAccess: true }),
...
try {
  ...
  const listPayload = await list.json() as { users: Array<{ id: string; subscription: { hasAccess: boolean } }>; total: number; limit: number; offset: number };
