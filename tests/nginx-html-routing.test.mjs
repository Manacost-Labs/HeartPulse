...
for (const route of inventory.routes) {
  ...
  if (['cosmetics-detail', 'bg-tier-list'].includes(route.id)) {
    expectRegexAction(`${path}/`, 'proxy_pass http://127.0.0.1:4321;', `${route.id} Next route`);
    const resolver = firstMatchingRegexLocation(`${path}/`);
...
async function startCardSeoUpstream() {
  const responses = new Map([
    ...
    ['/battlegrounds/tier-list/', {
      status: 200,
      headers: { 'Cache-Control': 'private, no-store' },
      body: '<!doctype html><title>Next BG tier list</title>',
    }],
    ['/battlegrounds/tier-list/not-a-page/', {
      status: 404,
      headers: { 'Cache-Control': 'private, no-store' },
    ...
    ['/legendaries/', {
      status: 200,
      headers: { 'Cache-Control': 'private, no-store' },
      body: '<!doctype html><title>Next legendaries</title>',
    }],
    ['/standard/matchups/', {
      status: 200,
      headers: { 'Cache-Control': 'private, no-store' },
    ...
    ['/standard/fun-decks/', {
      status: 200,
      headers: { 'Cache-Control': 'private, no-store' },
      body: '<!doctype html><title>Next fun decks</title>',
    }],
    ['/standard/vicious-gold/', {
      status: 200,
      headers: { 'Cache-Control': 'private, no-store' },
      body: '<!doctype html><title>Next Vicious Gold</title>',
    }],
    ['/standard/archetypes/', {
      status: 200,
      headers: { 'Cache-Control': 'private, no-store' },
...
`);
    ...
    const bgTierList = await requestNginx(port, '/battlegrounds/tier-list/');
    assert.equal(bgTierList.status, 200);
    assert.match(bgTierList.body, /<title>Next BG tier list<\/title>/);
    assert.equal(bgTierList.headers['x-robots-tag'], undefined);
    const bgTierRedirect = await requestNginx(port, '/battlegrounds/tier-list');
    assert.equal(bgTierRedirect.status, 301);
    assert.match(bgTierRedirect.headers.location || '', /\/battlegrounds\/tier-list\/$/);
    const bgTierFilter = await requestNginx(port, '/battlegrounds/tier-list/?list=strategies&source=hsreplay');
    assert.equal(bgTierFilter.status, 200);
    assert.equal(bgTierFilter.headers['x-robots-tag'], 'noindex, follow');
    ...
    for (const [path, expected] of [
      ...
      ['/standard/fun-decks/', /Next fun decks/],
    ...
    const failedLegendaries = await requestNginx(port, '/legendaries/?fail=1');
    assert.equal(failedLegendaries.status, 503, 'Arena legendaries upstream errors must retain their status');
    assert.equal(failedLegendaries.headers['x-robots-tag'], 'noindex, nofollow', 'Arena legendaries errors must not be indexed');
    ...
    const failedFunDecks = await requestNginx(port, '/standard/fun-decks/?fail=1');
    assert.equal(failedFunDecks.status, 503, 'Fun decks upstream errors must retain their status');
    assert.equal(failedFunDecks.headers['x-robots-tag'], 'noindex, nofollow', 'Fun decks errors must not be indexed');
    const failedGold = await requestNginx(port, '/standard/vicious-gold/?fail=1');
    assert.equal(failedGold.status, 503, 'Vicious Gold upstream errors must retain their status');
    assert.equal(failedGold.headers['x-robots-tag'], 'noindex, nofollow', 'Vicious Gold errors must not be indexed');
