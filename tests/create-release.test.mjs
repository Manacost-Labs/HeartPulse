...
const nginxContractFiles = [
  ...
  'deploy/nginx/arena-origin-real-ip.conf',
...
try {
  ...
  assert.deepEqual(
    ...
    [['origin'], ['origin'], ['origin'], ['origin'], ['edge'], ['edge'], ['edge'], ['edge'], ['edge'], ['edge'], ['edge'], ['origin'], ['origin']],
  );
  assert.deepEqual(
    ...
    [
      ...
      '/etc/nginx/conf.d/koloda-ru-proxy-realip.conf',
