...
export function ContestAdminStandardOperations({ onMessage }: { onMessage: (message: AdminMessage | null) => void }) {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || 'Не удалось загрузить состояние Standard');
      setStatus(payload as Status);
    } catch (error) {
      if (!signal?.aborted) onMessage({ type: 'err', text: error instanceof Error ? error.message : 'Не удалось загрузить состояние Standard' });
    } finally {
  ...
  return (
    <div className="admin-standard-operations">
      <div className="admin-stat-grid" aria-label="Состояние Traditional mode">
