...
function ArchetypeCatalogPage({
  ...
  const selectFormat = (next: ArchetypeFormat) => {
    setFormat(next);
    setQuery('');
    window.history.replaceState(window.history.state, '', `/standard/meta?format=${next}`);
  };
            ...
            {items.map((item, index) => (
              <article className="archetype-row" key={`${item.format}:${item.slug}`}>
                ...
                <a
                  ...
                  href={`/standard/meta/${item.format}/${item.slug}`}
                  onClick={event => {
                    event.preventDefault();
                    navigatePath(`/standard/meta/${item.format}/${item.slug}`);
                  }}
                >
                  <span>Открыть</span>
  ...
  if (error || !detail) {
    return (
      <main className="archetypes-page archetype-detail-page" id="main-content" tabIndex={-1}>
        <button type="button" className="archetype-back" onClick={() => navigatePath('/standard/meta')}>
  ...
  return (
    <main className="archetypes-page archetype-detail-page" id="main-content" tabIndex={-1}>
      <nav className="archetype-breadcrumb" aria-label="Навигационная цепочка">
        <a href="/standard/meta" onClick={event => { event.preventDefault(); navigatePath('/standard/meta'); }}>
...
export default function StandardMeta({
  ...
  const detailMatch = currentPath.match(/^\/standard\/meta\/(standard|wild)\/([a-z0-9-]+)\/?$/);
  ...
  return (
    <RecoverableSurfaceBoundary scope="standard-meta">
      {content}
    </RecoverableSurfaceBoundary>
  );
}
