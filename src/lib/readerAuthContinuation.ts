...
export function useReaderAuthState<User>(search: string) {
  const [appAuthUser, setAppAuthUser] = useState<User | null>(null);
  const [appAuthChecking, setAppAuthChecking] = useState(true);
  useEffect(() => {
    const next = readerAuthContinuation(search, () => window.sessionStorage, Boolean(appAuthUser));
    if (next) window.location.replace(next);
  }, [search, appAuthUser]);
  return { appAuthUser, setAppAuthUser, appAuthChecking, setAppAuthChecking };
}
