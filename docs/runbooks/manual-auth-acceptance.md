...
requires a real interactive terminal and an explicit `YES` before requests. It
can create a test account and send registration/login verification emails, so
run it only after separate authorization for those production actions. It is
explicitly excluded from the automated test registry.

The default target is HearthPulse; `BASE_URL` can select an HTTPS host or a
