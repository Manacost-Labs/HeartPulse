...
```dotenv
GAME_DATA_AUDIT_CODEX_ENABLED=0
...
Шаблон systemd оставляет AI-разбор выключенным (`0`). Его включение требует
явной настройки оператором. Режим `auto` включает эскалацию только при наличии
`auth.json` в отдельном
