...
Рабочий набор сейчас — Limburg и Москва; `www` наследует его через CNAME.
...
карантина задайте `HEARTHPULSE_MONITOR_QUARANTINED_REGION=novosibirsk` в
`/etc/hs-arena/hearthpulse-shadow-monitor.env` (root-only): монитор требует
точный двухузловой DNS-набор и проверяет оба оставшихся edge. Вернуть запись
можно после устранения тайм-аутов и браузерной проверки параллельной загрузки
CSS, JS и изображений через закреплённый новосибирский IP; затем убрать флаг
карантина и проверить полный трёхузловой монитор.
