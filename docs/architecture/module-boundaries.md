...
The live migration baseline is generated by `npm run agent:map` and enforced by
`npm run lint:module-boundaries`; volatile file and edge counts stay in that
machine-owned inventory instead of being copied into this document. Both checks
must report no missing public entries, outside-to-internal imports or runtime
...
`server.constructedCards` public entry now owns catalog-statistics normalization,
snapshot publishability checks and catalog/statistics merging alongside related
and generated-card enrichment.
