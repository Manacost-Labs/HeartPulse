...
shipped. Comments stay disabled. No live keys, services or account pages were created.
...
by email. Independent long-lived reader sessions will need an explicit security epoch.
...
are exposed in this DTO. `POST /reader-auth/logout` requires Origin and `X-Reader-CSRF`
and returns 204. Public HTML always contains an anonymous shell.
