...
Сделать публичную часть HearthPulse технически индексируемой, понятной поисковым системам и полезной пользователю до выполнения JavaScript. Одновременно нельзя раскрывать закрытую статистику тарифа «Алмаз» в HTML, sitemap, JSON-LD или Open Graph.
...
- `SEO-203/204`: `/sitemap.xml` публикует static, Standard, Wild-only, BG minion, BG spell и BG hero segments. Static-сегмент строится из SEO-реестра, а entity-сегменты используют те же проверенные публичные проекции, что SSR. Общий segment-scoped checksum-store с atomic `fsync`/rename и LKG отклоняет пустые, неполные, дублированные и резко усечённые кандидаты; `lastmod` меняется только вместе с semantic hash публичной сущности. Cache, singleflight, точный ETag, `Last-Modified`, безопасный LKG fallback и cold `503 Retry-After` закреплены unit/runtime tests.
...
2. Завершить оставшиеся дополнительные типы BG-сущностей только после появления полноценных SSR detail pages.
3. Добавить article segment только после появления локальных полных материалов с автором и датами.
...
- authoritative SSR и fail-closed поведение готовы для героев, существ и заклинаний active/archive; дополнительные типы BG-сущностей остаются вне scope `SEO-202`;
...
| SEO-202 | P0 | Аналогично покрыть `/heroes/:dbfId` и detail pages `/library/**` | SEO-101 | Герои и minion/spell details active/archive проходят 200/301/404/503/meta/schema/privacy/cache tests; дополнительные типы требуют отдельного среза |
| SEO-203 | P0 | Генерировать sitemap index и сегменты | SEO-201, SEO-202 | Готовы static + Standard + Wild-only + BG library + heroes; будущие articles публикуются только как 200 canonical URL |
| SEO-204 | P0 | Использовать реальный `lastmod` | Data publication metadata | Для всех текущих entity segments действует единый semantic-hash change-only contract |
...
Текущий статус фазы 2: `SEO-201` завершён для SSR detail pages Standard/Wild-карт. В `SEO-202` завершены hero и minion/spell details active/archive. `SEO-203/204` публикует static, Standard, Wild-only, BG minion/spell и hero segments через единый runtime index; каждый сегмент проходит отдельные ворота полноты, canonical/indexability и LKG.
