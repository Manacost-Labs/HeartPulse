...
- Release manifest schema v2 включает пять versioned nginx-файлов, их install path/роль origin или edge, SHA-256 и общий `nginxContract.hash`. Read-only verifier и pre-mutation deploy gate блокируют artifact/runtime drift; bootstrap/legacy/hash transition требуют явной проверки совместимости N/N-1.
...
|---|---|---|---|---|
| STAB-501 | P0 | Усилить существующий immutable deploy manifest | Текущий release flow | В работе: manifest v2 содержит git SHA/nginx contract, artifact/runtime drift проверяется до мутаций; остаются полный build hash, schema compatibility и dataset min/max version |
