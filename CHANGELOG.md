...
- Главная страница подготовлена в Next.js: лидеры классов и статьи попадают в
  серверный HTML без данных посетителя, а `/?login` остаётся доступным.
  Публичное переключение ожидает проверки релиза.
