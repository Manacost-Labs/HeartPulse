...
- Для новых героев Полей сражений Дест'агата и Кит'икса добавлены настоящие
  портреты вместо значка-заглушки в рейтинге и карточках героев.

- На страницах Полей сражений в Next.js восстановлено тематическое оформление
  фильтров, таблиц и карточек.
- Тир-лист Арены снова принимает реальные ответы источников данных: карточки
  не пропадают из-за различия между коротким ID и именем провайдера.

- Для московского edge добавлена проверка живости обратных SSH-туннелей:
  оборванное соединение больше не удерживает порт и не мешает восстановлению
  страниц и ассетов после сетевого сбоя. После проверки страниц, Next.js
  ассетов и CDN московский адрес возвращён в публичный DNS.

- После выкладки нового релиза прежние файлы CSS и JavaScript Next.js остаются
  доступными для уже открытых страниц в течение 35 дней.

- Страницы Next.js повторно проверяют сохранённую сессию и статус подписки
  после временных ошибок вместо ложного показа ограничения доступа подписчику.

- Origin записывает обезличенный тип источника успешных HTML-ответов в
  ограниченно хранимый журнал. Это позволит подтвердить семь дней без HTML
  из Vite перед удалением его сборки и инструментов.

- Текстовые баннеры разделов на мобильных экранах больше не занимают почти
  весь первый экран; содержимое тир-листа и других разделов видно раньше.

- Файлы JavaScript и CSS сборки Next.js теперь сжимаются на публичном Nginx
  после обработки origin-ответа; браузер загружает меньше данных на медленном
  соединении.

- Изображение верхнего баннера страницы героев Полей сражений теперь
  загружается заранее; это уменьшает время появления основного блока на
  медленном мобильном соединении без изменения оформления.

- Закрытые данные тир-листа Полей сражений загружаются только после
  подтверждения подписки; гостевая страница больше не загружает заранее
  тяжёлый код закрытого раздела.

- Для закрытого раздела архетипов подготовлены страницы Next.js: каталог
  Стандартного формата, числовые карточки и каталог Вольного формата с
  поиском, выбором архетипа и переходом к колоде в конструкторе. Права
  администратора проверяются до выдачи страницы и повторно в браузере;
  маршруты каталога и карточек переключены в Nginx на Next.js.

- Административный конструктор колод переключён на Next.js: сервер проверяет
  полные права администратора, а браузер повторяет проверку до загрузки
  редактора. Код колоды, черновик, выбор формата и класса сохраняют прежнее
  поведение; ответы страницы остаются приватными и без индексации.

- Неизвестные адреса теперь показывают общую страницу Next.js с тем же
  оформлением и навигацией, что у публичного сайта. Она возвращает HTTP 404;
  отдельная карточка карты сохраняет своё сообщение об ошибке. Nginx больше
  не берёт общий документ ошибки из сборки Vite.

- Страница администрирования переключена на Next.js: сервер проверяет права
  через действующую сессию, не включает данные пользователя в HTML и загружает
  рабочую панель только после повторной проверки в браузере. Ответы страницы
  остаются приватными, без индексации и кэширования.

- Нестабильные новосибирский и московский edge временно исключены из DNS
  HearthPulse после тайм-аутов загрузки страниц и ресурсов. Монитор проверяет
  оставшийся узел и не допускает случайного возврата неисправного IP в DNS.
  Дополнительная параллельная проба обнаруживает отказы загрузки ресурсов,
  которые не проявлялись при последовательных проверках.

- Публичные профили по адресам `/id/:id/` и `/profiles/:legacyId/`
  переключены на Next.js: серверный HTML содержит только разрешённые поля
  профиля, а неверные и отсутствующие идентификаторы возвращают 404.

- Страница подключения Manacost Tracker переключена на Next.js: код устройства
  не попадает в серверный HTML, а вход и подтверждение продолжают работать через
  существующий защищённый API. Неверные вложенные адреса возвращают 404.

- Конструкторы стратегий и тир-листов Полей сражений переключены на Next.js:
  публичные описания отделены от инструментов подписчиков, проверены импорт,
  экспорт PNG, сохранение настроек и ответы 404 для неверных адресов.

- Тир-лист Полей сражений переключён на Next.js: публичное описание отделено
  от статистики подписчиков, а выбранный раздел и источник сохраняются после
  обновления страницы. Проверены пустой ответ и ошибка источника данных.
