...
- Для неизвестных адресов подготовлена общая страница Next.js с тем же
  оформлением и навигацией, что у публичного сайта. Она возвращает HTTP 404;
  отдельная карточка карты сохраняет своё сообщение об ошибке. Переключение
  общего маршрута Nginx последует после проверки релиза.
