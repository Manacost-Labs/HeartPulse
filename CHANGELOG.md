...
- На Полях сражений сила героя появляется мягче при наведении и фокусе,
  ...
  выровнены; графики героя собраны во вкладки «Обзор», «Сила героя»,
  «Таверна» и «Составы», а мобильные карточки стали компактнее.

- Карты тир-листа Арены теперь сохраняют одинаковую рамку для изображений
  любого формата и fallback, с более мягким hover без потери rarity glow.
  Рейтинг классов больше не появляется построчно: заполнение шкалы занимает
  320 мс через transform, а контрастный процент остаётся читаемым с первого кадра.
