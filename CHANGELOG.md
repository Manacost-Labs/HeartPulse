...
- Страница администрирования подготовлена на Next.js: сервер проверяет права
  через действующую сессию, не включает данные пользователя в HTML и загружает
  рабочую панель только после повторной проверки в браузере. Переключение
  production-маршрута следует после отдельной проверки релиза.
