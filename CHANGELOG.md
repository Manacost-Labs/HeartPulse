...
- Документация Public API на `/developers/api/` подготовлена в Next.js с
  сохранением адреса, оформления и API-контракта; переключение маршрута
  ожидает проверки нового релиза на внутреннем порту.

- Перенос страниц на Next.js выпускается в два этапа: сначала код при прежнем
  Nginx-маршруте, затем проверенное переключение владельца URL.
