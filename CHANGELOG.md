...
- Каталоги и отдельные страницы скинов, монет и питомцев переключены на
  Next.js; проверены фильтры, прямые ссылки, SEO и ответы 404/503. End Screen
  питомцев теперь загружается через локальный прокси изображений.

- Страницы отдельных скинов, монет и питомцев подготовлены на Next.js:
  серверная карточка, SEO и ответы 404/503 проверены до переключения Nginx.

- Каталоги скинов, монет и питомцев подготовлены на Next.js: фильтры,
  канонические адреса и мобильная раскладка проверены до переключения.

- Все поддерживаемые страницы отдельных карт и архива Полей сражений
  переключены на Next.js; реальные ответы 404/503 и правила индексации сохранены.

- Остальные карточки библиотеки и архив Полей сражений подготовлены на Next.js:
  публичное описание, канонические адреса и ответы 404/503 проверены до переключения.

- Для переноса остальных карточек библиотеки Полей сражений подготовлен
  публичный API описаний активных и архивных карт без закрытой статистики.

- Для готовящихся страниц героев и карт Полей сражений в Next.js восстановлены
  структурированные данные с каноническими адресами и идентификаторами карт.

- Длинные кириллические адреса карт Полей сражений из sitemap открываются в
  Next.js без ошибочного 404.

- Детали героев, существ и заклинаний Полей сражений переключены на Next.js;
  канонические адреса, 404 и повторяемые 503 сохранены.

- Для будущего переключения деталей героев, существ и заклинаний на Next.js
  сохранены ответы 404 и 503 с повторной попыткой при недоступности каталога.

- Списки библиотеки Полей сражений, включая категории и архив, переключены
  на Next.js; страницы отдельных карт сохраняют прежние HTTP-статусы.

- Каталог героев Полей сражений переключён на Next.js после проверки
  гостевого доступа и загрузки статистики по подписке.

- Архив гайдов, страницы гайдов и детали архетипов переключены на Next.js
  после проверки гостевого доступа, подписки и прямой загрузки страниц.

- Страницы существ и заклинаний Полей сражений подготовлены на Next.js:
  публичное описание отделено от статистики, отсутствующие карты дают 404.

- Для переноса карточек существ и заклинаний подготовлено публичное превью
  без закрытой статистики, с проверкой отсутствующих карт и ошибок каталога.

- Остальные категории библиотеки Полей сражений, включая архивные, подготовлены
  на Next.js; неподдерживаемые архивы и неизвестные категории возвращают 404.

- Страницы существ, заклинаний и их архива подготовлены на Next.js;
  проверка подписки и канонические адреса сохранены.

- Библиотека Полей сражений подготовлена на Next.js: гостям доступно
  описание, а защищённые данные загружаются после проверки подписки.

- Карточка героя Полей сражений подготовлена на Next.js: публичная информация
  показывается в HTML, статистика остаётся закрытой, отсутствующий ID даёт 404.

- Каталог героев Полей сражений подготовлен на Next.js: гостям доступно
  описание, статистика загружается после проверки подписки или роли администратора.

- Страницы архивных гайдов подготовлены на Next.js: заголовок и короткое
  описание публичны, полный текст остаётся за проверкой доступа.

- Архив гайдов подготовлен на Next.js: его описание видно гостям, а список
  материалов загружается только после проверки доступа подписчика или администратора.

- Страницы архетипов и старые ссылки на детали меты подготовлены на Next.js:
  гостевое превью, проверка доступа и реальные ответы 404 сохранены.

- Сборка Storybook больше не копирует `public/` дважды и не падает из-за
  конфликта файлов при проверке релиза.

- Публичный каталог архетипов Стандарта переключён на Next.js; фильтры,
  гостевой предпросмотр и доступ подписчиков сохранены.

- Каталог архетипов подготовлен на Next.js: фильтры и гостевой каталог
  сохранены, а полные данные открываются подписчикам и администраторам.

- Администраторы видят полную подборку фан-колод в Next.js без отдельной
  подписки, как и в остальных разделах Стандарта.

- Администраторы снова видят закрытые страницы Арены, матчапов и меты в
  Next.js без отдельной подписки, как это разрешают API проекта.

- Публичная страница Vicious Syndicate Gold переключена на Next.js; платные
  данные по-прежнему загружаются только после проверки доступа.

- Страница Vicious Syndicate Gold подготовлена на Next.js: публичное описание
  доступно без подписки, а статистика загружается только после проверки доступа.

- Публичная страница фан-колод переключена на Next.js; фильтры и гостевой
  предпросмотр сохранены.

- Публичная страница меты Стандарта переключена на Next.js; гостевое превью
  и проверка подписки сохранены.

- Публичная страница матчапов Стандарта переключена на Next.js; проверка
  подписки и раздельный кэш аккаунта сохранены.

- Страница меты Стандарта подготовлена на Next.js: гостям доступно превью,
  полный набор статистики загружается после проверки подписки.

- Страница фан-колод подготовлена на Next.js: публичное описание доступно в
  HTML, фильтры и предпросмотр сохранены.

- При отзыве доступа к тир-листу или легендаркам Арены данные теперь исчезают
  с экрана после ответа API 401/403, а не остаются как устаревшие.

- Страница матчапов Стандарта подготовлена на Next.js: публичное описание
  доступно без подписки, а матрица загружается после проверки доступа.

- Публичная страница легендарных групп Арены переключена на Next.js;
  защищённый API и проверка подписки сохранены.

- Типы данных матчапов Стандарта вынесены в отдельный модуль перед переносом
  страницы на Next.js; текущая выдача страницы пока не менялась.

- Подготовлен клиент матчапов с раздельным кэшем для аккаунта и формата,
  проверкой данных и очисткой кэша и показанной матрицы после отказа в доступе.

- Загрузка матчапов подготовлена к смене аккаунта и формата без показа матрицы
  прежнего пользователя или неверного формата.

- Экран матчапов принимает отдельный источник данных для Next.js, не используя
  прежний общий кэш браузера в новом маршруте.

- Страница легендарок Арены подготовлена на Next.js: публичное описание
  доступно в HTML, личные группы загружаются после проверки подписки.

- Публичная страница тир-листа Арены переключена на Next.js; гостевой доступ,
  выбор источника и фильтрация карт-спутников сохранены.

- Контракт данных легендарных групп Арены вынесен в отдельный модуль для
  последующего переноса страницы на Next.js; текущий маршрут пока не менялся.

- Подготовлена загрузка легендарных групп с проверкой ответа и раздельным
  кэшем для аккаунта и источника; приватные данные очищаются при отказе доступа.

- Загрузка легендарных групп при смене аккаунта или подписки теперь скрывает
  данные прежнего пользователя до подтверждения нового доступа.

- Данные тир-листа Арены получили единый контракт типов и источников для
  поэтапного переноса страницы на Next.js. Новый клиент проверяет ответы,
  разделяет кэш по аккаунтам и сохраняет обработку устаревших данных.
  Страница подготовлена в Next.js и исключает карты-компаньоны при смене
  источника; публичный маршрут пока не менялся.

- Страница классов Арены переведена на Next.js: публичный текст доступен в
  серверном HTML, личная статистика загружается только после проверки подписки.
  Заголовок блока подписки не дублирует главный заголовок страницы.

- Пилот измерения затрат моделей теперь распознаёт сессии Claude Code через
  `CLAUDE_CODE_SESSION_ID` и учитывает их журналы без ручного поиска файла.

- Раздел конкурсов переведён на Next.js с серверным HTML. Правила участия,
  личные заявки и API остались за Express.

- Главная страница переведена на Next.js: лидеры классов и статьи попадают в
  серверный HTML без данных посетителя, а `/?login` остаётся доступным с
  запретом индексации состояния входа.

- Кнопка входа на страницах Next.js теперь открывает действующий адрес
  `/?login` вместо несуществующей страницы `/profile/`.

- Компоненты главной страницы вынесены в отдельный модуль для перехода на
  Next.js; текущий адрес и поведение страницы сохранены.

- Раздел статей переведён на Next.js: материалы есть в исходном HTML,
  фильтры сохраняют SEO-настройки, а личные голоса и VIP-доступ остаются
  за прежним API.

- Документация Public API на `/developers/api/` переведена на Next.js с
  сохранением адреса, оформления и API-контракта.

- Для нового владельца `/developers/api/` согласован проверенный Nginx-контракт
  выкладки, чтобы релиз проходил защищённый шлюз CI.

- Агентам добавлены локальный поиск существующего кода, расширенный набор
  проверочных запросов и учёт результатов задач для измерения экономии токенов.

- Каталог Стандарта пополняется анонсированными картами «Власти Темной империи»
  при каждом запуске импорта, даже если часть характеристик или изображение еще
  отсутствуют. Позднее данные дополняются без потери уже опубликованных карт.
  Проверка production принимает временные ID таких карт.

- Галерея артов переведена на Next.js: публичные карточки рендерятся на
  сервере, изображения и скачивание продолжают работать через прежний API,
  а канонический URL и метаданные превью ссылок сохранены.

- Восстановлен запуск страниц Next.js после перезагрузки сервера: сервис
  включён в автозапуск, инструкция развёртывания дополнена проверкой состояния.

- Добавлен проектный реестр проверок кода и компонентов для серверных quality
  tools; существующий шлюз релиза проверяет его команды и исходные файлы.

- CI включает скрытый каталог `.next` в проверенный артефакт релиза, чтобы
  запуск Next.js на проде получал собранные страницы и статические файлы.

- Каталог и страницы карт, FAQ, конфиденциальность и условия переведены на
  Next.js; остальные страницы и API продолжают обслуживаться прежним сервером.
  Ошибочные HTML-ответы Next.js получают заголовок `noindex, nofollow`.

- Production-релиз теперь включает Next.js для карт и юридических страниц.
  Отдельный процесс проверяется вместе с API при выкладке; при сбое Next
  переключение релиза откатывается.

- Браузерная проверка релиза в GitHub CI теперь использует установленный Google
  Chrome, чтобы запуск Chromium через обёртку не блокировал выкладку по таймауту.

- Службы резервного копирования, проверки и репликации теперь запускают скрипты
  через Bash: архив можно создать и проверить даже при неизменяемых правах релиза.

- Подготовлена контролируемая выкладка объединённых изменений: шлюз релиза
  разрешает переход конфигурации Nginx только для проверенного хеша. Сборка
  Next.js остаётся локальным пилотом до отдельного подключения в production.

- FAQ, политика конфиденциальности и условия использования добавлены в Next.js
  с прежним содержимым, серверным HTML и canonical. Общая оболочка сохраняет
  навигацию раздела карт; отдельный локальный флаг позволяет переключать и
  откатывать эти страницы независимо. Добавлены превью каталога и оболочки
  в Storybook. Production в этом этапе не переключался.

- Next.js обслуживает весь раздел карт при включении локального флага:
  каталоги Standard/Wild получают серверный HTML, canonical и JSON-LD. Поиск,
  фильтры, сортировка, вид и пагинация сохраняются в URL при перезагрузке,
  открытии карточки и возврате. Платная статистика остаётся за проверкой
  подписки в Express; отключение флага возвращает прежний frontend.

- Правила Nginx согласованы с юридическими страницами и Blizzard ID карт:
  canonical сохраняет `%3A`, параметры не теряются, двойное кодирование
  не превращается в другой действительный ID.

- Добавлен локальный пилот Next.js для публичных страниц карт: серверный HTML,
  canonical, JSON-LD, настоящие 404 и восстановление после сбоя каталога.
  Существующие интерфейс, адреса, фильтры, cookies и проверка подписки сохранены;
  приватные данные загружаются через защищённые Express API. Переключение
  раздела обратимо, production по умолчанию остаётся на прежнем frontend.
- Общие элементы навигации и политика фильтров карточек выделены из крупных
  компонентов; для завершённых доменных модулей включена строгая типизация.

- Резервный список героев Battlegrounds больше не остаётся в памяти навсегда:
  открытая страница повторяет запрос и возвращается к API после его восстановления.

- Рейтинг классов Арены больше не показывает демонстрационные проценты при
  сбое API. Добавлены отдельные состояния загрузки, пустого ответа, ошибки и
  сохранённых реальных данных, источник, время обновления и повторная загрузка.

- Блокировка аккаунта закрывает выдачу и обновление токенов приложения,
  доступ к профилю и запись событий трекера; отозванные токены не восстанавливаются
  после разблокировки.

- Регистрация и вход сохраняют только затронутые записи после отправки письма;
  параллельные запросы больше не заменяют чужие аккаунты, коды и сессии.

- Сохранён отдельный ручной тест авторизации: он требует терминал и явное
  подтверждение, не запускается в CI и отдельно запрашивает обязательное для регистрации
...
- Согласованы реестры модулей и тестов: проверки зависимостей, технического долга
  и завершения дочерних тестовых процессов действуют вместе.

- Пример настройки upstream API согласован с действующим адресом
  `api.kolodahearthstone.com`; сохранён отдельный шаблон systemd для явного
  применения этой настройки.

- Изолированный экземпляр можно запускать с `BACKGROUND_JOBS_ENABLED=0` без
  фонового обновления данных; обычный запуск сохраняет прежнее поведение.

- Объединены доработки проверки качества кода: учитывается полный диапазон
  коммитов CI, переименование файлов сохраняет ограничения технического долга,
  а расхождение установленного deploy-helper обнаруживается до выкладки.
- Исправлен актуальный пул существ Полей сражений для патча 36.6.1:
  удалённые карты перенесены в архив, возвращённые и новые добавлены.
  Библиотека и конструкторы используют один синхронизированный состав.

- Добавлена поддержка аберраций и новая иконка в библиотеке Полей сражений,
  тир-листах и конструкторах. Импортер сохраняет новый тип существ.

- Переходы между страницами стали мягче: навигация и фон остаются неподвижными,
  встречное движение старой и новой страницы убрано, а контент сменяется коротким
  перекрывающимся затуханием. Настройка уменьшения движения по-прежнему отключает
  анимацию.

- Из нижнего блока личного кабинета удалены кнопки «Открыть мету Standard · Beta»
  и «Настроить статьи»; кнопка выхода сохранена. Форма контактов больше не
  перекрывает выход при прокрутке на широком экране.

- Личный кабинет получил единый пергаментный фон независимо от предыдущего
  раздела. Обновлены шапка аккаунта, карточки доступа и контактов, увеличены
  поля и подписи. Способы привязки свёрнуты при активной подписке и открыты,
  когда доступ не подтверждён. Блок поддержки больше не перекрывает формы.

- При вводе в поиск и другие текстовые поля убраны золотое свечение,
  дополнительная рамка и изменение формы поля. Для перехода через Tab
  сохранён тонкий контур без свечения.
- Бюджет сжатого стартового JS учитывает обработчик способа ввода и остаётся
  ниже 80 КиБ; после удаления дублирующихся рамок снижен лимит стартового CSS.

- На странице меты убрана плашка ожидания обновления данных; кнопки формата
  и рейтинга выровнены по верхнему и нижнему краю. Арт шапок плавно переходит
  в бордовый фон на широких и мобильных экранах.
- Удалены оставшиеся требования к внешней базе задач и дизайн-доске из
  инструкций, документации и preflight. Контекст задают запрос пользователя,
  документы репозитория и визуальные референсы.

- Арт общих шапок выровнен по правому краю и масштабируется по высоте:
  на широких экранах лицо персонажа больше не обрезается, на мобильных
  арт опущен ниже заголовка и лучше виден под затемнением.

- Шапки традиционного режима, библиотеки карт и Арены приведены к общей
  ширине, отступам, масштабу заголовков и бордово-золотому оформлению.
  Убраны повторные отступы архетипов и разные мобильные высоты; длинные
  заголовки остаются видимыми. Под шапкой Меты убраны дублирующие ссылки,
  отступ до контента выровнен до 16 px. Добавлена браузерная проверка девяти
  экранов в обязательном release-наборе.

- Верхние баннеры традиционного режима, статей и архива гайдов теперь
  используют общую адаптивную высоту и отступы. Декоративная подсветка работает
  только через `opacity` и автоматически отключается при reduced motion, поэтому
  страницы выглядят цельно без лишней нагрузки на отрисовку.

- Легендарки больше не появляются каскадом после загрузки данных: список
  показывается сразу, без задержек между карточками.

- Убраны накладывающиеся анимации оболочки, загрузочного состояния и отдельных
  страниц, из-за которых часть разделов появлялась резко или собиралась по частям.

- Переходы между страницами приведены к единому плавному стилю без двойного
  мерцания; навигация назад и вперёд использует тот же переход.

- Карты дополнения «Власть Темной империи» в библиотеке теперь показывают
  полное русское название дополнения вместо внутреннего кода `BE`.

- Библиотека карт снова принимает актуальные идентификаторы карт Blizzard и
  автоматически обновляет каталог, поэтому не переходит в режим недоступности
  после обновления данных у поставщика.

- Внутренние ссылки теперь переключают маршруты без полной перезагрузки страницы:
  оболочка остаётся на месте, поддерживаемые браузеры показывают короткий плавный
  переход, а при reduced motion навигация остаётся мгновенной. Внешние ссылки,
  загрузки, API и реферальные переходы по-прежнему используют нативное поведение.

- Свернутое меню админ-панели стало ровной icon rail: пункты и разделители
  выровнены по одной сетке, а активный раздел отмечается тёмным полем и тонкой
  золотой линией. Готовые письма рассылки теперь используют фирменные пергамент,
  красное сукно и деревянную раму из локального набора материалов.

- Поля форм и основные действия админ-панели получили единые состояния фокуса
  и нажатия: контуры стали заметнее, а кнопки дают короткую тактильную отдачу
  без сдвига интерфейса. Настройка reduced motion сохранена.

- Раскрытое меню админ-панели получило более ясные маркеры групп и читаемую
  иерархию пунктов. Стартовая анимация раздела больше не делает заголовок
  невидимым для визуальных проверок.

- В админ-панели активный раздел больше не превращается в нечитаемую белую
  плашку: у него контрастный тёмный фон. Содержимое разделов получает короткое
  плавное появление при переключении, с отключением анимации для reduced motion.

- В интерфейсе карт скрыто служебное предупреждение о последнем сохранённом
  наборе данных: фактические ошибки и неполные данные по-прежнему показываются.
  В админ-панели убран дублирующий бренд из верхней строки, а левое меню теперь
  мягко раскрывается и использует более чистую компактную иконографику.

- Админ-панель получила новую рабочую визуальную систему: компактную icon rail,
  инструментальную верхнюю панель и единые карточки, таблицы и контролы с
  контрастной геометрией. Маршруты, права доступа и действия сохранены.

- В поиске героев Полей сражений убрано овальное нативное оформление браузера,
  а процент выбора в карточке выровнен чуть выше рядом со средним местом.

- Исправлена контрастность служебных подписей на странице героя и в списке
  заявок конкурса; мобильная карточка матчапа теперь не выходит за экран.

- На карточках героев Полей сражений сила героя теперь появляется с мягким
  замедлением и короткой естественной паузой после наведения; основной портрет
  плавно освобождает для неё место. Клавиатурный просмотр и reduced motion
  сохранены.

- Удалены закрытые административные страницы `/archetypes` и `/deck-builder`:
  они больше не отображаются в навигации, не пререндерятся и отвечают 404.
  Публичный раздел `/standard/archetypes`, API и внутренние данные сохранены.

- Главная страница HearthPulse теперь загружает нижние блоки и FAQ только при
  приближении к ним, а реестр обучения — после открытия «Помощи». Первый экран
  больше не конкурирует с этими невидимыми чанками за сеть и основной поток.

- Public-навигация HearthPulse стала выразительнее: desktop rail и mobile drawer
  получили контекстные подписи разделов, более ясные active/focus-состояния и
  Storybook-покрытие. Lazy-переходы теперь показывают компактный parchment
  fallback «Собираем раздел» и мягко появляются с поддержкой reduced motion;
  URL, права доступа, состав меню и предзагрузка маршрутов не изменились.
- В навигации HearthPulse заменены Manacost Stats и декоративные иконки: теперь
  используется фирменный fire-mark, а исходные игровые шрифты и иконки
  сохранены.

- Добавление и редактирование переводов в админ-панели больше не мерцает:
  подтверждение появляется сразу, а таблица и покрытие обновляются в фоне без
  исчезновения данных, скачка страницы или потери фокуса.

- Для production-кабинета Манакоста подготовлен отдельный точный клиент
  HearthPulse с 30-дневным входом по явному согласию. Адрес возврата ограничен
  `hs-manacost.ru`; старые и похожие идентификаторы, `.com`, `www` и изменённые
  URL отклоняются. Мосты прав администратора и платной подписки принимают только
  точные Reader-клиенты и остаются выключенными до отдельной активации.

- Админ-панель стала короче и понятнее: из навигации убраны «Аналитика» и
  «Сочетания в Арене», а «Пользователи», «Конкурсы», «Рассылка», «Реферальная
  ссылка» и «Переводы» получили единые операционные сводки, ясные основные
  действия и удобную мобильную компоновку.

- Админ-панель стала быстрее и устойчивее: тяжёлая аналитика загружается только
  при открытии раздела, проверка переводов больше не ждёт внешние коды колод, а
  неполные ответы аналитики и Арены показывают локальную ошибку вместо падения
  всей страницы. Верхняя панель стала компактнее, а browser QA теперь обходит
  все 16 административных разделов на компьютере и телефоне.

- Админ-панель HearthPulse получила новый операционный интерфейс: фирменную
  навигацию, честную сводку на реальных данных и таблицу запусков парсеров с
  доступной панелью деталей. API, права доступа и логика публикации не менялись.

- Edge-публикатор принимает текущий проверенный размер frontend-бандла, сохраняя
  ограничение, которое блокирует неполные деревья статики.
...
- Стили анимации рейтинга классов загружаются вместе с его компонентом,
  без увеличения общей таблицы стилей остальных страниц.

- В разделах статистики героя без данных показывается явное пояснение.

- На Полях сражений сила героя появляется мягче при наведении и фокусе,
  поддерживает Escape и уменьшенное движение. Обычный и золотой компаньоны
  используют актуальные полноразмерные версии карт и одинаковую рамку;
  графики героя собраны во вкладки «Обзор», «Сила героя», «Таверна»
  и «Составы», ограничены читаемой шириной и не выходят за страницу.

- Карты тир-листа Арены теперь сохраняют одинаковую рамку для изображений
  любого формата и fallback; hover стал мягче и больше не анимирует тяжёлый
  фильтр. Шкалы рейтинга классов плавно заполняются через transform с лёгким
  каскадом, а контрастный процент остаётся читаемым с первого кадра.

- Для тестового кабинета Манакоста подготовлено сохранение входа до 30 дней:
  с явным согласием, короткими токенами доступа и безопасным обновлением на
  сервере. Выход или отзыв исходной сессии HearthPulse прекращает доступ раньше;
  обычная авторизация HearthPulse и WordPress не меняется.

- Зарезервированный staging-клиент Манакоста теперь может использоваться в
  production identity только с явным флагом и точным callback тестового сайта;
  повторное использование его идентификатора с production callback блокируется
  ещё при запуске.

- Добавлен отключённый по умолчанию конфиденциальный пакетный мост актуальных
  Reader-полномочий для модерации комментариев: он читает только текущие
  `users.role`/`blocked_at`, не добавляет OIDC claims и не меняет paid-доступ.

- Подготовлен отключённый по умолчанию конфиденциальный пакетный запрос статуса
  платной подписки для комментариев: учитываются только свежие подтверждения
  Boosty и Patreon, без Telegram и ручных выдач доступа.

- Соединения входа с Манакоста изолированы от общего прокси-пула: проверка
  сертификата больше не зависит от ранее открытого соединения другого домена.
  Добавлена регрессионная проверка с настоящим TLS.

- Добавлен явный режим входа тестового Манакоста через основной HearthPulse:
  только точный адрес возврата, отдельные ключи и данные кабинета, приватные
  маршруты без кеширования. Очистка истёкших записей сохраняет защиту от повторов.

- Подготовлен первый отключённый по умолчанию вариант входа читателей Манакоста:
  подтверждение в HearthPulse, возврат после входа, зашифрованные сессии и отзыв
  доступа при завершении исходной сессии. Тестовый вариант ограничен пятью
  минутами без фонового продления. На действующих сайтах он ещё не включён;
  текущая авторизация и отключённые комментарии не изменены.
