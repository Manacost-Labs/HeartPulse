...
- [ ] Phase 3: layered CSS and removal of legacy cascade.
  ...
  - [x] Profile save success/error notices now use semantic roles and one route-owned visual cascade instead of inline styles plus 15 forced declarations; deterministic browser QA submits both responses at desktop/mobile widths and the global ceiling is 1,786.
  - [x] Card lightboxes now take their timber frame, red tavern texture, stats surface and close-button material from the shared canonical lightbox owner instead of two deferred-route duplicates; mobile E2E locks the computed asset URLs/materials alongside background scroll restoration, the global ceiling falls to 1,763, and deferred route CSS drops from 66.9 to 65.3 KB under a new 67 KB budget.
  - [x] Card lightbox positioning, responsive grid, reveal transition, image sizing and 44px close target now live in semantic CSS instead of five inline style owners; browser QA rejects inline regressions, 32 more forced declarations are removed, route JS falls from 113.5 to 112.3 KB and its budget tightens to 116 KB.
  - [x] The deferred Tier List and Legendaries lightbox now owns its visual cascade outside the legacy utility layer, removing 58 more forced declarations; desktop, compact-height and mobile browser QA lock its two/one-column geometry, canonical materials and viewport containment, and CI tightens the global ceiling to 1,673 plus deferred route CSS to 66 KB.
  - [x] The Arena card hover preview now owns its timber frame, parchment body, red header and stat rows outside the legacy utility layer, removing 28 more forced declarations; desktop and compact-height QA verify its computed materials and viewport containment, while CI tightens the global ceiling to 1,645 and deferred route CSS to 65.5 KB.
  - [x] Classes, Tier List and Legendaries now take their red timber, parchment switcher, active-filter and ranked-card materials from the canonical route owner instead of an obsolete blue utility theme plus a 70-declaration compensating duplicate; desktop/mobile computed-style QA locks the rendered contract, 113 forced declarations are removed, the global ceiling falls to 1,532 and deferred route CSS drops to 54,831 bytes under a 55 KB budget.
  - [x] The oldest Profile workspace layer now provides only normal structural defaults, while the later parchment/reliquary owners control its final materials; 140 redundant force flags and the obsolete active-subscription divider override are gone, desktop/mobile E2E locks every primary frame, status chip and form input, the global ceiling falls to 1,390 and deferred route CSS to 53,156 bytes under a 53.5 KB budget.
  - [x] The authenticated Profile workspace no longer carries inline presentation or two competing parchment themes: semantic classes own its structure and one material layer owns every frame, hero, status chip and responsive state. Browser QA preserves desktop/mobile interaction, reflow and zero axe violations; CI rejects inline-style/theme regressions, lowers the global force ceiling to 1,219 and caps deferred route CSS at 52 KB after reducing it to 51,659 bytes.
  - [x] Each lazy Home directory now has a local error boundary: a failed articles, Battlegrounds or Arena chunk can no longer remove the rest of the page. Deterministic mobile E2E aborts the articles chunk and requires the remaining landmarks, zero horizontal overflow and a 44 px recovery action while the existing initial JS/CSS budgets stay unchanged.
  - [x] Public login, registration and password recovery now share one semantic route-owned material layer instead of inline blue presentation fighting a forced parchment override. Desktop/mobile E2E covers every mode, deterministic error feedback, labelled fields, 44 px actions, reflow and zero axe violations; CI rejects returning inline styles, caps the lazy auth CSS at 4.5 KB and lowers the global force ceiling to 1,182.
  - [x] The Profile/auth responsive route shell now owns its zero-padding mobile canvas and bounded content gutter through normal unlayered specificity instead of three forced declarations; mobile profile E2E locks the computed padding/max-width contract and the global force ceiling falls to 1,179.
  - [x] Deferred Arena card modals now own their wooden frame, red tavern material, responsive geometry and missing-image fallback in one lazy stylesheet; shared parchment CSS cannot reclaim those selectors, inline presentation is forbidden and the global force ceiling falls to 1,152.
  - [x] Locked Battlegrounds routes now load a lightweight parchment shell with the subscription gate instead of waiting for a blocked data component to mount; direct guest desktop/mobile E2E rejects legacy white canvas strips, requires continuous parchment coverage and captures visual regression evidence while heavy Battlegrounds component CSS remains lazy.
...
- [ ] Phase 4: modular API, runtime validation and durable data snapshots.
  ...
  - [x] Authentication and newsletter mail enter Exim through a bounded, injection-safe loopback SMTP transport; the service keeps `NoNewPrivileges=true`, SMTP diagnostics stay private and regression coverage forbids the incompatible setuid sendmail path.
  ...
  - [x] Archetype translations now live in a durable SQLite table managed from a responsive admin workspace: bounded bilingual search and pagination, manual create/edit priority, audited CSRF-protected mutations and transactional BlizzCore synchronization are covered by HTTP contracts plus desktop/mobile zero-axe E2E; Standard matchup labels consume the managed table with existing fallbacks retained for upstream failure.
